...
func (r *runner) executePlan(ctx context.Context, selected AvailablePlan, planIndex int) planResult {
	...
	if err != nil {
		planRes.Failed = true
		planRes.FailureReason = fmt.Sprintf("create plan failed: %v", err)
		planRes.FinishedAt = time.Now().UTC()
		planRes.Duration = planRes.FinishedAt.Sub(planRes.StartedAt).String()
	...
	if len(modules) == 0 {
		planRes.Failed = true
		planRes.FailureReason = "plan contains no executable modules after filtering"
		planRes.FinishedAt = time.Now().UTC()
		planRes.Duration = planRes.FinishedAt.Sub(planRes.StartedAt).String()
		r.logf("plan failed: %s (%s)", selected.Name, planRes.FailureReason)
		return planRes
...
func (r *runner) executeModule(ctx context.Context, selected AvailablePlan, module PlanModule, planID string, planIndex, moduleIndex int) testResult {
	...
	if err != nil {
		res.Status = "ERROR"
		res.Result = "FAILED"
		res.Summary = fmt.Sprintf("create test instance failed: %v", err)
	...
	if err != nil {
		res.Status = "ERROR"
		res.Result = "FAILED"
		res.Summary = fmt.Sprintf("poll failed: %v", err)
		res.FinishedAt = time.Now().UTC()
		res.Duration = res.FinishedAt.Sub(res.StartedAt).String()
		r.logf("  test failed: module=%s err=%v", module.Name, err)
		return res
