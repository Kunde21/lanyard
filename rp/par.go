...
func (r *RP) buildAuthorizationParameters(state, nonce, verifier, challenge, authorizationDetails string, resources []string, extra url.Values) url.Values {
	params := url.Values{}
	...
	if strings.TrimSpace(r.responseMode) != "" {
		params.Set("response_mode", r.responseMode)
	}
